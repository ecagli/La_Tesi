...
\begin{itemize}
Random \emph{masks}: $M_1, \dots , M_{d-1}$  
\emph{Masked} variable: $M_d = Z \oplus M_1^{-1}\oplus \dots \oplus M_{d-1}$
\end{itemize}
...
\begin{block}{Problem}
Naive strategy: useful statistics lie in a high-dimensional \emph{feature} space: 
...
\emph{"Fisher Discriminant Analysis with Kernels"(\cite{scholkopf1999fisher})}
\begin{center}
\only<1>{
...
\item \uncover<2->{Memory-based [$\extract^{\mathrm{KDA}}_{\ell}(\vec{x}) =$ \textcolor{red}{$\sum_{i=1}^{\NPoI}$}$\nununu_\ell[i]$\textcolor{red}{$K(\sss[z_i]{i}, \sss[]{})$}]\uncover<3>{$+$ $O(N^3)$ complexity $\rightarrow$ Non-scalability to big training set}}
\end{itemize}
