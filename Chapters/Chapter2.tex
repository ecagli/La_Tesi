...
Throughout this thesis, and each time a profiling attack scenario is supposed,  we will refer to elements of $\sensVarSet$ as \emph{classes}. We will say that acquired traces associated to a same value $\sensVar\in\sensVarSet$ \emph{belong} to the same class $\sensVar$. Eventually we will use the term \emph{label} to denote the classes, in such a way that each trace is \emph{labelled} by an element of $\sensVarSet$, or associated to a label $\sensVar\in\sensVarSet$.
...
The goal of these researches is to study and develop techniques to characterize PoIs and to apply conve- nient dimensionality reduction techniques, that allow reducing the size of the acquisitions while keeping the exploitable information held by data high enough to allow an SCA to succeed.
